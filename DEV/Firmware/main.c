...
{
	vga_cntrl_red_write(15);
	vga_cntrl_green_write(15);
	vga_cntrl_blue_write(15);
	
	while(!(buttons_in_read()&1<<0)) {
		if(vga_cntrl_hpos_read()<100) {
			vga_cntrl_red_write(0);
			vga_cntrl_green_write(0);
		}
		else {
			vga_cntrl_red_write(15);
			vga_cntrl_green_write(15);
	...
	/*
	vga_cntrl_memwe_write(1);
	for(i=0; i<480*640; i++) {
		vga_cntrl_memadr_write(i);
		vga_cntrl_memdatw_write(1);
	}
	vga_cntrl_memwe_write(0);
	for(i=0; i<480*640; i++) {
		vga_cntrl_memadr_write(i);
		printf("%d\t",vga_cntrl_memdatr_read());
	}
	*/
}
